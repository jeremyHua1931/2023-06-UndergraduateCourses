...
0000000000400efc <phase_2>:
  ...
  400f02:	48 89 e6             	mov    %rsp,%rsi     //将栈指针保存在%rsi
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers>   //调用函数,读取六个数字
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)          //将栈指针的内存值与1进行比较
  400f0e:	74 20                	je     400f30 <phase_2+0x34>   //等于1，跳转到0x400f30

  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>   //不等于1，爆炸
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>   
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax         //由400f3a的值调转到此处时，将%rsp -4 的值赋给%eax
  400f1a:	01 c0                	add    %eax,%eax               //%eax值扩大两倍
  400f1c:	39 03                	cmp    %eax,(%rbx)            //将%rbx的值与%eax比较
  400f1e:	74 05                	je     400f25 <phase_2+0x29>   //相等，调转到  0x400f25
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>    //不等，爆炸
  400f25:	48 83 c3 04          	add    $0x4,%rbx               //%rbx =%rbx+4
  400f29:	48 39 eb             	cmp    %rbp,%rbx               //将%rbx的值与%rbp比较
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>    //%rbx<=%rbp，跳转到 0x400f3c
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>     //大于跳转到0x400f3c

  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx       //由400f0e跳转到此处时，将%rsp +4的值赋给%rbx
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp      //将%rsp +24 的值赋给%rbp
  400f3a:	eb db                	jmp    400f17 <phase_2+0x1b>   //跳转到0x400f17
  400f3c:	48 83 c4 28          	add    $0x28,%rsp       //栈指针上移40个字节，
  400f40:	5b                   	pop    %rbx         
  400f41:	5d                   	pop    %rbp
  400f42:	c3                   	retq   
...
0000000000400f43 <phase_3>:
  ...
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8)       //跳转到0x4070+ 8*%rax 的地址
  400f7c:	b8 cf 00 00 00       	mov    $0xcf,%eax               //%eax=207
  400f81:	eb 3b                	jmp    400fbe <phase_3+0x7b>
  400f83:	b8 c3 02 00 00       	mov    $0x2c3,%eax             //%eax=707
  400f88:	eb 34                	jmp    400fbe <phase_3+0x7b>      //跳转到0x400fbe
  400f8a:	b8 00 01 00 00       	mov    $0x100,%eax               //%eax=256
  400f8f:	eb 2d                	jmp    400fbe <phase_3+0x7b>      //跳转到0x400fbe
  400f91:	b8 85 01 00 00       	mov    $0x185,%eax              //%eax=389
  400f96:	eb 26                	jmp    400fbe <phase_3+0x7b>      //跳转到0x400fbe
  400f98:	b8 ce 00 00 00       	mov    $0xce,%eax              //%eax=206
  400f9d:	eb 1f                	jmp    400fbe <phase_3+0x7b>      //跳转到0x400fbe
  400f9f:	b8 aa 02 00 00       	mov    $0x2aa,%eax                //%eax=682
  400fa4:	eb 18                	jmp    400fbe <phase_3+0x7b>      //跳转到0x400fbe
  400fa6:	b8 47 01 00 00       	mov    $0x147,%eax              //%eax=327
  400fab:	eb 11                	jmp    400fbe <phase_3+0x7b>    //跳转到0x400fbe
  400fad:	e8 88 04 00 00       	callq  40143a <explode_bomb>
  400fb2:	b8 00 00 00 00       	mov    $0x0,%eax                //%eax=0
  400fb7:	eb 05                	jmp    400fbe <phase_3+0x7b>    //%eax=311
...
0000000000400fce <func4>:               //func(%edi=%rsp+8,%esi=0,%edx=14)
  ...
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24> //%ecx（=7）<=%edi,跳转至0x400ff2,其中%edi=%rsp+8的值
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx   //否则%edx=%rcx-1=6
  400fe9:	e8 e0 ff ff ff       	callq  400fce <func4>    //调用函数func4 ,func4(%edi=%rsp+8,%esi=0,%edx=6)
  ...
  400ffe:	e8 cb ff ff ff       	callq  400fce <func4>     //调用函数func4,func4(%edi=%rsp+8,%esi=9,%edx=14)
...
0000000000401062 <phase_5>:
  ...
  4010b8:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi    //将%rsp+8的值赋给%rdi
...
00000000004010f4 <phase_6>:
  ...
  4010fa:	55                   	push   %rbp
  4010fb:	53                   	push   %rbx
  4010fc:	48 83 ec 50          	sub    $0x50,%rsp   //栈指针下移80的字节
  ...
  401130:	74 21                	je     401153 <phase_6+0x5f>  //% r12=6,跳转到0x401153
  ...
  401145:	83 c3 01             	add    $0x1,%ebx           //%ebx=%ebx+1
  ...
  *****************************  /*将所有的数变为 7-原来的数*/ **************************************
  ...
  4011ab:	48 8b 5c 24 20       	mov    0x20(%rsp),%rbx    //将%rsp+32 的值赋给%rbx
  4011b0:	48 8d 44 24 28       	lea    0x28(%rsp),%rax   //%rax= %rsp+40的值
  4011b5:	48 8d 74 24 50       	lea    0x50(%rsp),%rsi   //%rsi= %rsp+80的值 (地址处的值)
  ...
  4011df:	48 8b 43 08          	mov    0x8(%rbx),%rax    //将%rbx 的值 赋给%rax
...
0000000000401242 <secret_phase>:
  ...
  401278:	83 f8 02             	cmp    $0x2,%eax     //返回值与2比，相等就返回
