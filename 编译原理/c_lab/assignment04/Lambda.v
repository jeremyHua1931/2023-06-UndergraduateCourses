...
Fixpoint cbv t :=
  ...
  | <\v : t1> => let t1' := cbv t1 in
             ...
             | Some t1'' => Some  <\v : t1''> 
               ...
               (* if t1 is ABS and t2 is ABS, then 
                  not evalute the t2 (function argument is delayed) *)
  | <t1  t2> => match t1 with
                | <\v1 : t1'> => match t2 with
                               | <\v2: t2'> => Some (substitution t1' v1 t2)
                               | _ =>  let t2'' := cbv t2 in
                                      match t2'' with
                                      | None => 
                                           Some (substitution t1' v1 t2)
                                      | Some t2''' => Some <t1 t2'''>
                                      end
                               end
                 | _ =>  let t2'' := cbv t2 in
                                      match t2'' with
                                      | None => match cbv t1 with
                                               | None => None
                                               | Some t1' => Some <t1' t2>
                                               end
                                      | Some t2''' => Some <t1 t2'''>
