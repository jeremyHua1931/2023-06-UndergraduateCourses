...
AST *clone_tree(AST *source)
{
  while (source != NULL)
  {
    ...
    {
    case CONST:
      return make_const(source->value);
    case VAR:
      return make_var(source->value);
    case COND:
      return make_cond(clone_tree(source->cond),
                       clone_tree(source->lchild),
                       clone_tree(source->rchild));
    case ABS:
      return make_abs(NULL, clone_tree(source->rchild));
    default:
...
CLOSURE *get_argument(int n, CLOSURE_LIST *env, int index)
{
  ...
  if (i == n - 1 && env != NULL)
    return clone_clos(env->clos);

  if (index - (n - i) >= 0)
    return get_global(index - n + i);
  /* n + i is top of stack */
